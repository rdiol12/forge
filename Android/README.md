...
Download `Forge-android.apk` from the repository's private [Releases](https://github.com/rdiol12/forge/releases). Allow installation from your browser/file manager when Android asks, then install the APK. Release APKs use a persistent signing key, so later builds update the existing app. The debug app uses a separate application ID and can be installed alongside it.

The shared [iOS and Android workflow](https://github.com/rdiol12/forge/actions/workflows/ios.yml) builds both platforms from the same commit with the same build number. Android runs unit tests, lint, emulator UI/Keystore checks, private repository/workflow/release reads, and real private release/Actions-artifact downloads. It also launches the optimized release APK, captures its Home screen and verifies its signature/checksum. Only after both platforms pass, one private release publishes `Forge-android.apk`, `Forge-unsigned.ipa` and a shared `SHA256SUMS`. A failed platform blocks publication. Pushes to `main` and manual runs create `build-<run>-<attempt>` prereleases; `v*` tags create normal releases. Documentation-only branch pushes are skipped, and existing releases are never overwritten.
