...
- Choose a repository in Actions, then filter its failures/active runs and search its workflow history.
...
New in 0.8: repository cards with rich README images/tables and editing; Home issue creation with assignees, labels, milestones and Projects; real pinned repositories and supported profile badges; profile editing and follow/unfollow; Actions grouped by project; releases sorted newest first; a bounded, account-isolated 30-second memory cache; visible local download deletion; and paginated commit history with undo, removal and recovery controls.

Commit text merging is conservative: separate changed spans merge automatically, while overlapping or complex edits require your choice of a complete version or edited text (up to 1 MiB). Every choice is tied to the exact file revisions; the branch stays unchanged until all conflicts are resolved and you confirm again.

**Deleted commits:** removal saves a local record before the branch changes, grouped by repository under Home ? Deleted commits and Repository ? More. Forge stores commit IDs and metadata, **not a permanent backup or a recovery tag**. Records belong to the connected account on that device. Restore the exact saved history only while the branch is unchanged; Reapply creates a new commit while preserving later work when files do not conflict. Recovery requires GitHub to retain the objects and your token to retain access. No retention deadline is promised. Clearing app data/uninstalling loses local records; removals made outside Forge are not automatically discovered. Failed removal attempts may also leave a record.

History updates use Git's expected-old-SHA check, enforced by the server. Branch protection remains in force. Root/merge rewrites, file/directory collisions, more than 200 later commits, truncated trees or large rewrite plans require desktop Git. Rewriting changes later commit IDs and loses their original signatures; it does not purge copies or sensitive data. [GitHub commit recovery](https://docs.github.com/en/pull-requests/how-tos/commit-changes/troubleshooting-commits), [Git reference update protocol](https://git-scm.com/docs/gitprotocol-pack).

Profile editing supports name, bio, website, company, location, social handle and hiring availability. GitHub's API exposes Staff, Developer Program, Star and Campus Expert flags; it does not expose every profile achievement badge or provide avatar/username/security editing through these profile endpoints. [Profile API](https://docs.github.com/en/rest/users/users), [GraphQL User](https://docs.github.com/en/graphql/reference/objects#user).
...
Public release browsing and downloads work without signing in. **Sign in with GitHub** in Settings uses the public login service; activation still requires OAuth credentials configured on the [login backend](Backend/). Under Advanced, you can connect a [fine-grained personal access token](https://github.com/settings/personal-access-tokens/new) limited to the repositories you need, with **Actions: Read-only** and **Contents: Read-only**. GitHub may require organization approval. Enter the token in the app; never commit it to this repository. With manual sign-in, GitHub Inbox requires a [classic token](https://github.com/settings/tokens/new) with `notifications` (or `repo` for private repositories); GitHub does not support fine-grained tokens for the [notifications API](https://docs.github.com/en/rest/activity/notifications).
...
- Native collaboration supports issue creation, issue/PR/Discussion watching, PR reviews, review-thread resolution, and direct merging. Merge queues/auto-merge and editing merge conflicts are not implemented. Repository and profile READMEs render images/tables; issue/release Markdown uses the simpler native renderer. GitHub search caps results at 1,000 and pull-request files at 3,000; large/binary diff patches may be omitted.
...
Feature requests are researched from [GitHub Community Mobile discussions](https://github.com/orgs/community/discussions/categories/mobile). Internal research notes stay local.
...
The memory cache is capped at 16 MiB/100 responses, respects cache-control, and uses ETags where supplied. Manual refresh, writes and account changes invalidate it. Git history writes authenticate only to the fixed github.com Git endpoint and refuse redirects; other API credentials stay on api.github.com. README images in the same repository load at the selected SHA through the authenticated API; external images never receive the token. Deleting a saved download removes its local copy, not its GitHub release. Deleting a release removes its uploaded files but leaves its Git tag.

0.8 local validation (2026-09-24): 54 Swift core tests pass, including a real local bare-Git stale-update rejection, failed recovery persistence, cache invalidation/ETag revalidation and profile conflict checks. Android unit tests, compilation and lint pass. GitHub mutations are stubbed or tested in local Git; physical-device history writes and full browser OAuth onboarding still require device validation.

Profile/follow changes require the classic `user` scope or the corresponding fine-grained account permissions. Projects need `read:project` for browsing and `project` for assignment. Workflow-file edits may need `workflow`. The read-only live check verified private README HTML/PNG, actual pins and commit/issue metadata; its developer token lacks `read:project`, and the permission error is surfaced without blocking issue creation.
